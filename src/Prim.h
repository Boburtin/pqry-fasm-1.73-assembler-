...
inline usize aligned_size(usize n) { return (n + kCacheLine - 1) & ~(kCacheLine - 1); }
...
struct TokArray {
    ...
    void grow() {
        ...
        kind = alloc_array<TokKind>(
            aligned_grow(kind, cap * sizeof(TokKind), aligned_size(n * sizeof(TokKind)), kCacheLine), n);
        start = alloc_array<u32>(
            aligned_grow(start, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        end = alloc_array<u32>(
            aligned_grow(end, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        cap = n;
    }
...
struct DefTable {
    ...
    void grow() {
        ...
        kind = alloc_array<DefKind>(
            aligned_grow(kind, cap * sizeof(DefKind), aligned_size(n * sizeof(DefKind)), kCacheLine), n);
        valBeg = alloc_array<u32>(
            aligned_grow(valBeg, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        valEnd = alloc_array<u32>(
            aligned_grow(valEnd, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        lock = alloc_array<u32>(
            aligned_grow(lock, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        prev = alloc_array<u32>(
            aligned_grow(prev, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        cap = n;
    }
...
struct ParamStore {
    ...
    u32 *nameLen{};
    ParamMode *mode{};
    u32 *defltBeg{}; // [defltBeg,defltEnd) window into MacroTable::body;
                     // U32_MAX = no default
    u32 *defltEnd{};
    ...
    void grow() {
        u32 n = cap ? cap * 2 : 32;
        nameOff = alloc_array<u32>(
            aligned_grow(nameOff, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        nameLen = alloc_array<u32>(
            aligned_grow(nameLen, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        mode = alloc_array<ParamMode>(
            aligned_grow(mode, cap * sizeof(ParamMode), aligned_size(n * sizeof(ParamMode)), kCacheLine),
            n);
        defltBeg = alloc_array<u32>(
            aligned_grow(defltBeg, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        defltEnd = alloc_array<u32>(
            aligned_grow(defltEnd, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        cap = n;
    }
...
struct MacroTable {
    ...
    void grow() {
        ...
        kind = alloc_array<MacroKind>(
            aligned_grow(kind, cap * sizeof(MacroKind), aligned_size(n * sizeof(MacroKind)), kCacheLine),
            n);
        paramBeg = alloc_array<u32>(
            aligned_grow(paramBeg, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        paramEnd = alloc_array<u32>(
            aligned_grow(paramEnd, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        bodyBeg = alloc_array<u32>(
            aligned_grow(bodyBeg, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        bodyEnd = alloc_array<u32>(
            aligned_grow(bodyEnd, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        cap = n;
    }
...
struct OriginArray {
    ...
    void grow() {
        u32 n = cap ? cap * 2 : 256;
        srcTok = alloc_array<u32>(
            aligned_grow(srcTok, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        depth = alloc_array<u32>(
            aligned_grow(depth, cap * sizeof(u32), aligned_size(n * sizeof(u32)), kCacheLine), n);
        cap = n;
    }
